...
def do_forward(options, free_port):
    """
    ...
    context.verbose('Connecting to ssh host %s:%d ...' % (options.server[0], options.server[1]))
    try:
        ...
        sys.exit(1)
    context.verbose('Now forwarding port %d to %s:%d ...' % (free_port, options.remote[0], options.remote[1]))
    try:
        forward_tunnel(free_port, options.remote[0], options.remote[1], client.get_transport())
...
def multi_forward():
    ...
    try:
        for remote_node in remote_nodes:
            ...
            forward_local_ports.append({"name": remote_node.name, "free_port": free_port})
